...
fn main() -> anyhow::Result<()> {
    let out = std::env::args()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("runs/anthropic-wrapper.jsonl"));

    let mut start = EventInput::new("run_anthropic_wrapper_example", 1, "run.start");
    start.action = Some("trigger.api".to_owned());
    start.resource = Some("example://anthropic-wrapper".to_owned());
    ...
    llm_call.action = Some("anthropic.messages.create".to_owned());
    llm_call.resource = Some("anthropic://model/example".to_owned());
    ...
    println!("Anthropic-style run log written to {}", out.display());
    Ok(())
}
